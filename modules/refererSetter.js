...
function checkScriptRefresh(channel) {
  ...
  if(type != types.TYPE_DOCUMENT && type != types.TYPE_SUBDOCUMENT)
    return;
  ...
  if(!browser && channel.notificationCallbacks) {
    ...
    var loadCtx = channel.notificationCallbacks.QueryInterface(Components.interfaces.nsIInterfaceRequestor).getInterface(Components.interfaces.nsILoadContext);
    browser = loadCtx.topFrameElement;
  }
  
  var windowId = channel.loadInfo.innerWindowID;
  
  GM_util.getService().scriptRefresh("any", channel.URI.spec, windowId, browser); 
}
